<template>
  <div class="app-container">
    ...
    <ul class="chapterList">
      <li v-for="(chapter,index) in chapterList" :key="index">
        <p>
          {{ chapter.title }}
          <span class="acts">
        ...
        <ul class="chapterList videoList">
          <li v-for="(video, indexJ) in chapter.children" :key="indexJ">
            <p>
              {{ video.title }}
...
export default {
  ...
  methods: {
    ...
    // 添加章节
    addChapter() {
      // 调用 chapter-form ref="chapterForm"/> 的 open 方法
      this.$refs.chapterForm.open()
    },
    // 获取后端章节列表数据
    fetchNodeList() {
      chapterApi.getNestedTreeList(this.$parent.courseId).then(resp => {
    ...
    removeChapterById(chapter) {
      this.$confirm('此操作将永久删除【' + chapter.title + '】章节，是否继续?', '提示', {
        ...
        return chapterApi.removeById(chapter.id)
      }).then(resp => {
        this.fetchNodeList()
